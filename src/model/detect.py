...
from src.Dataset import continue_learning_Dataset
...
def get_feature(data, graph, args, model, adj):
    ...
    if args.raw:
        node_size = data.shape[2]
        data = data.reshape((-1,data.shape[2]))
    else:
        node_size = data.shape[1]
    data = np.reshape(data[-args.window*7-1:-1,:], (-1, args.x_len, node_size))
...
def get_adj(year, args):
    #adj = np.load(osp.join(args.graph_path, str(year)+"_adj.npz"))['arr_0']
...
def influence_node_selection(model, args, pre_data, cur_data, pre_graph, cur_graph, type):
...
def influence_node_selection_promax(model, args, pre_data, cur_data, pre_graph, cur_graph):
    model.eval()
    pre_adj = get_adj(args.year-1, args)
    cur_adj = get_adj(args.year, args)
    L = np.array(range(pre_adj.size(0)))  
    pre_feature = get_feature(pre_data, pre_graph, args, model, pre_adj)
    cur_feature = get_feature(cur_data, cur_graph, args, model, cur_adj)
    pre_feature = torch.from_numpy(pre_feature)
    ...
    nodes= torch.tensor([]).to(args.device, non_blocking=True)
    for i in L:
        score = torch.tensor([]).to(args.device, non_blocking=True)
        for j in args.neighbors[i]:
            scorei = (distance.jensenshannon(pre_feature[i],cur_feature[i])*distance.jensenshannon(pre_feature[j],cur_feature[j]))/(distance.jensenshannon(cur_feature[i],cur_feature[j])*distance.jensenshannon(pre_feature[i],pre_feature[j]))     
            scorei = torch.from_numpy(scorei).to(args.device, non_blocking=True)
            score = torch.cat([scorei,score]) 
        nodes = torch.cat([nodes,torch.norm(score).view(1)])
    return np.argpartition(np.asarray(nodes.cpu()), args.topk)[:args.topk], np.argpartition(np.asarray(nodes.cpu()), args.topkR)[-args.topkR:]

def influence_node_selection_pro(model, args, pre_data, cur_data, pre_graph, cur_graph):
    model.eval()
    pre_adj = get_adj(args.year-1, args)
    cur_adj = get_adj(args.year, args)
    L = np.array(range(pre_adj.size(0)))  
    ...
    for i in L:
        ...
        for j in args.neighbors[i]:
            ...
            score = torch.cat([scorei,score]) 
        nodes = torch.cat([nodes,torch.norm(score).view(1)])
    return np.argpartition(np.asarray(nodes.cpu()), args.topk)[:args.topk]

def finfluence_node_selection_pro(model, args, pre_data, cur_data, pre_graph, cur_graph):
    model.eval()
    pre_adj = get_adj(args.year, args)
    cur_adj = get_adj(args.year+1, args)
    L = np.array(range(pre_adj.size(0)))  
    pre_feature = get_feature(pre_data, pre_graph, args, model, pre_adj)
    cur_feature = get_feature(cur_data, cur_graph, args, model, cur_adj)
    pre_feature = torch.from_numpy(pre_feature)
    ...
    for i in L:
        score = torch.tensor([]).to(args.device, non_blocking=True)
        for j in args.cneighbors[i]:
            scorei = (distance.jensenshannon(pre_feature[i],cur_feature[i])*distance.jensenshannon(pre_feature[j],cur_feature[j]))/(distance.jensenshannon(cur_feature[i],cur_feature[j])*distance.jensenshannon(pre_feature[i],pre_feature[j]))     
            scorei = torch.from_numpy(scorei).to(args.device, non_blocking=True)
            score = torch.cat([scorei,score]) 
        nodes = torch.cat([nodes,torch.norm(score).view(1)])
    return np.argpartition(np.asarray(score.cpu()), args.topk)[:args.topk]

def clean(list,args):
    return [x for x in list if min(args.fneighbors[x])<args.graph_size or x < args.graph_size]
