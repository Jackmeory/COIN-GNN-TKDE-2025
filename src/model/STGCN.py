...
class STGCN(AbstractModel):
    def __init__(self, args):
        super().__init__(args)
        self.num_nodes = args.adj.size(0)
        self.feature_dim = 1
        self.output_dim = 1
    ...
    def forward(self, data):
        x = data['X']  # (batch_size, input_length, num_nodes, feature_dim)
    ...
    def predict(self, data):
        # 多步预测
        x = data.x.reshape((-1, self.input_window, self.num_nodes, self.feature_dim)) # (batch_size, input_length, num_nodes, feature_dim)
        y = data.y.reshape((-1, self.input_window, self.num_nodes, self.feature_dim))   # (batch_size, output_length, num_nodes, feature_dim)
        y_preds = []
        x_ = x.clone()
        ...
        y_preds = y_preds.reshape((-1, self.output_window))
        return y_preds
